<template>
  <div class="pref-keybindings">
    <h4>{{$t('preferences.category.key-bindings-option.key-bindings')}}</h4>
    <section class="keybindings">
      <div class="text">
        {{$t('preferences.category.key-bindings-option.key-bindings-description')}}
      <a class="link" @click="openKeybindingWiki">{{$t('preferences.category.key-bindings-option.online')}}</a>.
      </div>
      <el-table
        :data="keybindingList"
        style="width: 100%"
      >
        <el-table-column prop="description" :label="$t('preferences.category.key-bindings-option.description')">
        </el-table-column>
        <el-table-column prop="accelerator" :label="$t('preferences.category.key-bindings-option.key-Combination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('preferences.category.key-bindings-option.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('preferences.category.key-bindings-option.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('preferences.category.key-bindings-option.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('preferences.category.key-bindings-option.unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{$t('preferences.category.key-bindings-option.save')}}</el-button>
      <el-button size="medium" @click="restoreDefaults">{{$t('preferences.category.key-bindings-option.restore')}}</el-button>
    </section>
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{$t('preferences.category.key-bindings-option.debug-options')}}:</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{$t('preferences.category.key-bindings-option.dumpKeyboardInformation')}}</el-button>
    </section>
    <key-input-dialog
